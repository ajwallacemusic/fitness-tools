...
```bash
curl -s localhost:8080/v1/tools/body-fat \
  -X POST -H 'content-type: application/json' \
  -d '{"sex":"male","neck":{"value":40,"unit":"cm"},"waist":{"value":90,"unit":"cm"},"height":{"value":180,"unit":"cm"}}'
```

  </TabItem>
  <TabItem label="MCP">

Call the `body-fat` tool with these arguments:
